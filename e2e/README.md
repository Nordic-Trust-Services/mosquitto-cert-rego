...
| `auth_rbac_matrix_allow_and_deny` | Walks examples/11_rbac_matrix.rego end-to-end: mints six certs with different role/fleet URN sets, then asserts 10 positive allow cases + 7 explicit negative cases (viewer publish denied, operator lacks config, operator publish outside `ops/cmd/ack/`, device publishing under another device's CN, device subscribing to `$SYS/`, admin without fleet membership, admin crossing tenants) + 1 connect-deny canary for an unknown role. Assertions drive off the audit log, not `mosquitto_pub` exit codes (QoS-1 PUBACK fires even on ACL-denied publishes). |
